...
const (
	...
	repoEnvVar     = "BOOP_REPO"
	appcastURL     = "https://raw.githubusercontent.com/fayazara/boop/main/appcast.xml"

	// Homebrew tap (cask) configuration.
	tapRepo      = "fayazara/homebrew-tap"
	caskRelPath  = "Casks/boop.rb"
	tapDirEnvVar = "BOOP_TAP_DIR"
	bundleID     = "com.fayazahmed.Boop"

	// Build/notarize configuration (used with -build).
	projectName     = "Boop.xcodeproj"
	releaseScheme   = "Boop"
	developmentTeam = "TB2S44TFQS"
	archiveName     = "Boop.xcarchive"
)
...
func main() {
	...
	// notarytool profiles are per Apple *team*, not per app, so Boop reuses the
	// existing one rather than duplicating the same credentials under a new name.
...
func updateHomebrewCask(homeDir, version, dmgPath string) error {
	...
	commitMsg := fmt.Sprintf("boop %s", version)
...
// BOOP_TAP_DIR (a local clone) or falls back to a temporary clone.
...
func renderCask(version, sha string) string {
	return fmt.Sprintf(`cask "boop" do
  version "%s"
  sha256 "%s"
...
func findRepoDir(homeDir string) string {
	...
	candidates := []string{
		filepath.Join(homeDir, "Developer", "fayazara", "mac", "Boop"),
	}
	...
	fail("Could not find Boop repo. Set " + repoEnvVar + " to the repo path.")
	return ""
}
...
// (~/Downloads/Boop.app) so the rest of the pipeline can package it.
func runBuildPhase(repoDir, homeDir, appPath string) {
	...
	exportDir, err := os.MkdirTemp("", "boop-export-")
	if err != nil {
	...
	zipPath := filepath.Join(exportDir, "Boop-notarize.zip")
...
<rss version="2.0"
  ...
  <channel>
    <title>Boop Updates</title>
    <link>https://raw.githubusercontent.com/fayazara/boop/main/appcast.xml</link>
    <language>en</language>

    <!--
      HOW TO ADD A NEW RELEASE:
      1. Bump MARKETING_VERSION and CURRENT_PROJECT_VERSION in Xcode.
      2. Export Boop.app to ~/Downloads.
      3. Run: go run ./cmd/boop-release
         - Enter release notes when prompted, or run non-interactively with
           flags, e.g.:
             go run ./cmd/boop-release -yes \
               -notes "Fixed pixelate preview
             Improved upload flow"
           (-notes-file <path> reads bullets from a file instead.)

      The release tool creates Boop.dmg, signs it with Sparkle, prepends
