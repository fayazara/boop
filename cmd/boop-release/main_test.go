...
func TestWriteAppcast(t *testing.T) {
	...
	items := []Item{
		{
			...
			Enclosure: Enclosure{
				URL:         "https://github.com/fayazara/boop/releases/download/v1.0/Boop.dmg",
	...
	text := string(data)
	if !strings.Contains(text, "<title>Boop Updates</title>") {
		t.Fatal("appcast title was not written")
	}
	if !strings.Contains(text, "Run: go run ./cmd/boop-release") {
